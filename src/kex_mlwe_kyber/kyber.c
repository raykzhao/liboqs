...
static void keygen(unsigned char *pk, unsigned char *sk, OQS_RAND *rand) {
	...
	tester_OQS_SHA3_shake128(sk + KYBER_SECRETKEYBYTES - 64, 32, pk, KYBER_PUBLICKEYBYTES);
...
static void sharedb(unsigned char *sharedkey, unsigned char *send,
	...
	tester_OQS_SHA3_shake128(buf, 32, buf, 32); /* Don't release system RNG output */

	tester_OQS_SHA3_shake128(buf + 32, 32, received, KYBER_PUBLICKEYBYTES); /* Multitarget countermeasure for coins + contributory KEM */
	tester_OQS_SHA3_shake128(krq, 96, buf, 64);
	...
	tester_OQS_SHA3_shake128(krq + 32, 32, send, KYBER_BYTES); /* overwrite coins in krq with h(c) */
	tester_OQS_SHA3_shake128(sharedkey, 32, krq, 64);          /* hash concatenation of pre-k and h(c) to k */

#ifndef STATISTICAL_TEST
	tester_OQS_SHA3_sha3256(sharedkey, sharedkey, 32);
#endif
}
...
static void shareda(unsigned char *sharedkey, const unsigned char *sk,
	...
	tester_OQS_SHA3_shake128(krq + 32, 32, received, KYBER_BYTES); /* overwrite coins in krq with h(c)  */
